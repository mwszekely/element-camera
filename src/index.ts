 ...
 */
export function roundByDevicePixels(x: number, step: number, op = Math.round) {
...
export class ElementCamera {
    ...
    onFocusChange?: (e: PointerEvent | WheelEvent, x: number, y: number) => void = undefined;
    onScaleChange?: (newScale: number) => void = undefined;
    onClick?: (e: PointerEvent, x: number, y: number) => void = undefined;

    constructor(public element: HTMLElement, initialStep = 1) {
     ...
     * no click event.)
     */
    private pannedOrZoomedDuringPointerDown: undefined | boolean = undefined;
     ...
     */
    private pointerAction: null | 'pending' | 'pan' | 'zoom' = null;
     ...
     */
    private pointerData: Map<number, { startX: number; startY: number; currentX: number; currentY: number }> = new Map();
    ...
    private _recalculateImpl() {
        ...
        let nextStateToCommit: ElementCameraState = {
            scaleUsed: nextScale,
        ...
        if (
            nextStateToCommit.scaleUsed != this.lastCommittedState.scaleUsed ||
